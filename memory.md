...
| 30 | **video1 Wiener img4 Exp-C 50k** | 3850 | 1020×574 | MCMC 1M | 50k | 16.12 | 0.645 | 0.588 VGG / **0.846** Alex | GATE 1 FAIL (>0.7). GPU cache mode. Relocate declined 980k→700k (not classic fog pattern). LPIPS-Alex anomalously high vs VGG. eval.json `E:\vksplat_output\video1_v1_wiener_img4_50k_exp_c\20260520_003505_video1\eval.json` |
| 31 | **video1 Restormer img4 Exp-C 50k** | 3850 | 1020×574 | MCMC 1M | 50k | 17.25 | 0.739 | 0.469 VGG / **0.703** Alex | MUCH better than Wiener (0.703 vs 0.846 Alex). Barely misses 0.7 gate. eval.json `E:\vksplat_output\video1_v1_restormer_img4_50k\20260520_012536_video1\eval.json` |
| 32 | **video1 NAFNet img4 Exp-C 50k** | 3850 | 1020×574 | MCMC 1M | 50k | 11.57 | 0.226 | 0.661 VGG / **0.835** Alex | BAD — PSNR 11.57, SSIM 0.23. Likely degenerate. eval.json `E:\vksplat_output\video1_v1_nafnet_img4_50k\20260520_021619_video1\eval.json` |
| 33 | **video1 Unsharp img4 Exp-C 50k** | 3850 | 1020×574 | MCMC 1M | 50k | 10.18 | 0.138 | 0.655 VGG / **0.840** Alex | BAD — PSNR 10.18, SSIM 0.14. Degenerate. eval.json `E:\vksplat_output\video1_v1_unsharp_img4_50k\20260520_030458_video1\eval.json` |
| 34 | **video1 Raw img4 Exp-C 50k** | 3850 | 1020×574 | MCMC 1M | 50k | 12.55 | 0.360 | 0.612 VGG / **0.738** Alex | Control: raw is 2nd-best LPIPS-Alex, better than NAFNet/Unsharp/Wiener. eval.json `E:\vksplat_output\video1_v1_raw_img4_50k\20260520_081101_video1\eval.json` |
| — | ⚠️ **GPU image cache bug found + fixed** (2026-05-20) | — | — | — | — | — | — | — | **`--image-cache-device gpu` produced LPIPS-Alex 0.877 (fog) on lr_full 100k — a config that gets 0.365 in CPU mode. Root cause: in GPU mode, each image has its own VkBuffer handle. `executeCompute` caches descriptors by buffer list; when the image buffer changes each step, it triggers `HOST_GUARD` (submits the command batch + fence) to update descriptors. This splits the forward pass and SSIM into separate Vulkan command batches, breaking the `bufferMemoryBarrier` that synchronizes pixel_state (COMPUTE_SHADER_WRITE→READ) and the reference image (TRANSFER_WRITE→READ) — those barriers only synchronize within a single command buffer. Fix: GPU mode copies the per-image device buffer to a shared `ref_image` buffer via `copyFromDeviceToDevice` (device-to-device, same command batch), then uses the stable `ref_image` handle for SSIM. Descriptor set stays constant → no HOST_GUARD → no batch split → barriers work. Verified: GPU-fixed LPIPS-Alex 0.362 matches CPU baseline 0.365.** |
| 35 | **lr_full 100k GPU-cache BROKEN** | 1318 | 1000×462 | MCMC 1M, ssim_λ 0.4 | 100k | 14.93 | 0.486 | 0.661 VGG / **0.877** Alex | ❌ FOG. Pre-fix `--image-cache-device gpu`. Proves GPU mode was broken. `E:\vksplat_output\lr_full_lr_full_100k_gpu_cache_regtest\20260520_090017_lr_full\eval.json` |
| 36 | **lr_full 100k CPU control** | 1318 | 1000×462 | MCMC 1M, ssim_λ 0.4 | 100k | 21.08 | 0.681 | 0.386 VGG / **0.366** Alex | ✅ Matches baseline (0.365). Proves CPU mode unaffected. `E:\vksplat_output\lr_full_lr_full_100k_cpu_cache_regtest\20260520_092758_lr_full\eval.json` |
| 37 | **lr_full 100k GPU-cache FIXED** ⭐ | 1318 | 1000×462 | MCMC 1M, ssim_λ 0.4 | 100k | **21.08** | **0.681** | 0.383 VGG / **0.362** Alex | ✅ **GPU cache works after fix.** Matches CPU baseline. `E:\vksplat_output\lr_full_lr_full_100k_gpu_fixed\20260520_101427_lr_full\eval.json` |
| — | 🔚 **GPU cache regression test PASSED** (2026-05-20) | — | — | — | — | — | — | — | **Verified: `--image-cache-device gpu` with the copyFromDeviceToDevice fix produces identical results to CPU mode (LPIPS-Alex 0.362 vs 0.366, within MCMC noise). All video1 runs (rows 30-34) used the BROKEN GPU mode — their poor scores may be partly attributable to this bug (though video1's inherent blur is the dominant factor). The fix also frees ~2.4 GB CPU RAM for lr_full (or ~9 GB for video1) by clearing CPU copies after upload.** |
| — | 🔄 **video1 RERUN with fixed GPU cache** (2026-05-20) | — | — | — | — | — | — | — | **All 5 video1 runs repeated with the Vulkan GPU cache fix. 4/5 improved significantly, 1 (Restormer) had MCMC collapse on first attempt (retry recovered). The bug was inflating LPIPS scores by 0.04–0.13 on average.** |
| 38 | **video1 Wiener FIXED** | 3850 | 1020×574 | MCMC 1M | 50k | 17.09 | 0.659 | 0.524 VGG / **0.770** Alex | Was 0.846 → **0.770** (-9%). `E:\vksplat_output\video1_v1_wiener_fixed\20260520_104945_video1\eval.json` |
| 39 | **video1 Restormer FIXED** (retry) | 3850 | 1020×574 | MCMC 1M | 50k | 17.17 | 0.740 | 0.498 VGG / **0.741** Alex | Was 0.703 → **0.741** (+5%). First attempt collapsed (PSNR 8.10, MCMC bad luck). Retry is slightly worse than broken run — Restormer was actually the LEAST affected by the bug. `E:\vksplat_output\video1_v1_restormer_fixed2\20260520_145330_video1\eval.json` |
| 40 | **video1 NAFNet FIXED** ⭐ | 3850 | 1020×574 | MCMC 1M | 50k | **17.62** | **0.741** | 0.470 VGG / **0.703** Alex | Was 0.835 → **0.703** (-16%). PSNR 11.57→17.62. **BIGGEST improvement — was degenerate, now viable.** `E:\vksplat_output\video1_v1_nafnet_fixed\20260520_122716_video1\eval.json` |
| 41 | **video1 Unsharp FIXED** | 3850 | 1020×574 | MCMC 1M | 50k | **17.90** | **0.707** | 0.505 VGG / **0.744** Alex | Was 0.840 → **0.744** (-11%). PSNR 10.18→17.90. **Also was degenerate, now viable.** `E:\vksplat_output\video1_v1_unsharp_fixed\20260520_131601_video1\eval.json` |
| 42 | **video1 Raw FIXED** ⭐⭐ | 3850 | 1020×574 | MCMC 1M | 50k | **17.84** | **0.752** | 0.460 VGG / **0.696** Alex | Was 0.738 → **0.696** (-6%). **PASSES <0.7 GATE.** Best LPIPS-Alex of all video1 variants. `E:\vksplat_output\video1_v1_raw_fixed\20260520_140444_video1\eval.json` |
| — | 🔚 **video1 FIXED SWEEP RANKING** (2026-05-20) | — | — | — | — | — | — | — | **New ranking: Raw 0.696 ⭐ > NAFNet 0.703 > Restormer 0.741 > Unsharp 0.744 > Wiener 0.770. Winner: RAW (plain images, no restoration). Only Raw passes the <0.7 gate. Key finding: the broken GPU mode inflated scores worst for NAFNet (+0.132) and Unsharp (+0.096), completely masking their true quality. Restoration tiers are now much closer (0.696–0.770 range vs 0.703–0.846 broken). Restormer, which appeared best in the broken sweep (0.703), is now mid-pack (0.741). The raw images, with no preprocessing, produce the best 3DGS reconstruction — restoration HURTS on this dataset.** |
